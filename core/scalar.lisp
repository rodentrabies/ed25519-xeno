...
(defun sc-from-bytes (sbytes)
  ...
  (let ((s      (make-array 24 :element-type 'int64))
        ...
        (coeffs (make-array 6 :element-type 'int32 :initial-contents +clist+)))
...
(defun sc-mul-add (abytes bbytes cbytes)
  (let ((a      (make-array 12 :element-type 'int64))
    ...
    (setf (aref s 20) (+ (* (aref a 9) (aref b 11)) (* (aref a 10) (aref b 10))
                         (* (aref a 11) (aref b 9))))
    (setf (aref s 21) (+ (* (aref a 10) (aref b 11)) (* (aref a 11) (aref b 10))))
    (setf (aref s 22) (+ (* (aref a 11) (aref b 11))))
    (setf (aref s 23) 0)
    ;; TODO(mrwhythat): prepare for encoding
    (sc-encode s)))

(declaim (ftype (function ((simple-array int64)) scalar) sc-encode))
(defun sc-encode (s)
  "Encode expanded representation of scalar into a 32-byte array."
  (let ((r (make-array +scalar-size+ :element-type '(unsigned-byte 8))))
